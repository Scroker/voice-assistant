...
class TestJSSyntax(unittest.TestCase):
    ...
    def test_quick_toggle_is_owned_by_system_indicator(self):
        """Il toggle deve essere gestito correttamente all'interno del SystemIndicator."""
        filepath = os.path.join(ROOT_DIR, "src", "extension.js")
        with open(filepath, "r", encoding="utf-8") as source_file:
            source = source_file.read()

        self.assertIn("this.quickToggle = new VoiceAssistantQuickToggle(extension);", source)
        self.assertIn("this.quickSettingsItems.push(this.quickToggle);", source)
        self.assertIn("this.quickToggle.destroy();", source)
